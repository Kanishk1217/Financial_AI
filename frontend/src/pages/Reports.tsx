...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--warm-border-strong)', borderRadius: 10, padding: '10px 14px', fontSize: 12 }}>
      <p style={{ color: 'var(--ink-muted)', marginBottom: 4 }}>{label}</p>
      <p style={{ color: 'var(--ink)', fontWeight: 700 }}>${Number(payload[0].value).toLocaleString()}</p>
    </div>
  )
}
...
export default function Reports() {
  ...
  const selectStyle = (key: string) => ({
    padding: '9px 14px', borderRadius: 11, fontSize: 13, color: 'var(--ink)', outline: 'none',
    background: 'var(--surface)',
    border: '1px solid var(--warm-border-strong)',
     cursor: 'pointer', color: 'var(--ink)', transition: 'border-color 0.2s, background 0.2s',
  })
  ...
  return (
    <div style={{ background: 'var(--parchment)', padding: '36px 36px 48px', position: 'relative', minHeight: '100vh' }}>
      ...
      {/* Decorative glows */}
      <div style={{ position: 'absolute', top: 0, left: '20%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(24,21,16,0.02) 0%, transparent 65%)', filter: 'blur(60px)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: 0, right: '-5%', width: 450, height: 450, borderRadius: '50%', background: 'radial-gradient(circle, rgba(24,21,16,0.015) 0%, transparent 65%)', filter: 'blur(60px)', pointerEvents: 'none' }} />
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={handleExport} disabled={exporting || !report}
              style={{
                ...
                border: '1px solid var(--warm-border-strong)', cursor: exporting || !report ? 'not-allowed' : 'pointer',
                background: 'var(--parchment)', color: 'var(--ink)', fontSize: 13, fontWeight: 600,
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
              {[...Array(4)].map((_, i) => <div key={i} style={{ height: 100, borderRadius: 18, background: 'var(--surface-hover)' }} className="shimmer-line" />)}
            </div>
            <div style={{ height: 260, borderRadius: 20, background: 'var(--surface-hover)' }} className="shimmer-line" />
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p style={{ fontSize: 13, color: 'var(--ink-secondary)', maxWidth: 260, lineHeight: 1.6 }}>Add transactions to see your monthly report</p>
          </motion.div>
        ) : (
          <>
            {/* Stat cards */}
                ...
                <motion.div key={s.label}
                  ...
                  <GlowCard padding={18} radius={18}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                      ...
                      <div style={{ width: 28, height: 28, borderRadius: 9, background: 'var(--parchment-deep)', border: '1px solid var(--warm-border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <s.icon size={13} style={{ color: 'var(--ink)' }} />
                      </div>
                    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              ...
              <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
                <GlowCard padding={22} radius={20}>
                  <h2 style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-muted)', marginBottom: 16, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Category Breakdown</h2>
                      ...
                      return (
                        <div key={c.name}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>${c.value.toLocaleString()}</span>
                          </div>
                          <div style={{ height: 8, borderRadius: 4, background: 'var(--parchment)', overflow: 'hidden' }}>
                            <motion.div
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
              <GlowCard padding={22} radius={20}>
              <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-muted)', marginBottom: 18, letterSpacing: '-0.01em' }}>Top Merchants</h2>
              <h2 style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-muted)', marginBottom: 16, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Top Merchants</h2>
                  ...
                  return (
                    <motion.div key={m.name}
                      ...
                      <span style={{
                        ...
                        background: 'var(--parchment)', border: '1px solid var(--warm-border)',
                        color: 'var(--ink)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        flexShrink: 0,
                      }}>{i + 1}</span>
                      <span style={{ fontSize: 13, color: 'var(--ink)', fontWeight: 500, width: 160, flexShrink: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.name}</span>
                      <div style={{ flex: 1, height: 6, background: 'var(--parchment)', borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
                        <motion.div
                          initial={{ width: 0 }} animate={{ width: `${pct}%` }}
                      ...
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)', width: 90, textAlign: 'right', flexShrink: 0 }}>${m.total.toLocaleString()}</span>
                    </motion.div>
                  )
                })}
              </div>
